...
#clearing flow
while converge == 1 and iteration < 1000:
    #Initializing iteration
    iteration = iteration + 1
    trialSet = random.sample(s,1)
#    print(trialSet)
    ub_iter_set = {}     
    #Forward propoation
    ...
    for week in w:
        if week == w[0]:
            ...
            for trial in trialSet:
                ...
                newCuts = {}
                CutCount = 0
                hyd.RHS = swinflow[trial,week]*5+resPrevious[week,trial]
                m.update()
                dem.RHS = wdemand[week] - wexchange[week]
                m.update()
                for f in ft:
        ...
        for week in w[::-1]:
            ...
            for scen in s:
                ...
                if week == w[0]:
                    ...
                    hyd.RHS = swinflow[scen,week]*5+resPrevious[week,trial]
                    m.update()
                dem.RHS = wdemand[week] - wexchange[week]
                m.update()
                for f in ft:
...
#    sigma_iter[iteration]= math.sqrt((sum(((lb_iter[iteration]-ub_iter_set[iteration,trial])**2) for trial in trialSet))/(len(trialSet)-1))
#    if abs(lb_iter[iteration]-ub_iter[iteration]+1.96*sigma_iter[iteration])/math.sqrt(len(trialSet))<0.1*abs(lb_iter[iteration]):
