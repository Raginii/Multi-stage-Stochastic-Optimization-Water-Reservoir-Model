...
res = m.addVars(Nlist,name = 'res') ##reservoir energy level at end in T
...
m.addConstrs(((res[i] - res[ancestorN[i]] + x[i,'Hydro'] + spill[i] == swinflow[scenN[i],timeN[i]]) for i in n) , "Hydraulic continuity" )
...
m.setObjective(
        (
        quicksum(gencost[i,f]*x[i,f] for i in n for f in ft if (timeN[i],f) in capacityKeys)
